...
<div class="col-md-12"> <h2>Add Group Members</h2>
...
<input type="hidden" name="groupName" value="<%=request.getAttribute("groupName").toString().trim() %>"/>
 ...
 <c:forEach var="userdsc" items="${lst}">
		<tr><td>${userdsc.getName() }</td>  
			...
			<td>
			<form method="post" action="RegGroupMember">
			<input type="hidden" name="groupName" value="<%=request.getAttribute("groupName").toString().trim() %>"/>
			<input type="hidden" name="userid" value="<%=session.getAttribute("userid").toString().trim() %>"/>
			
			<input type="hidden" value="${userdsc.getUserid() }" name="memUserid"/>
			<input type="hidden" value="${userdsc.getName() }" name="memUserName"/>
